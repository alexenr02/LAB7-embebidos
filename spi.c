 ...
 * spi.c
 *
 * Created: 15/10/2021 02:05:43 p. m.
...
void spiInit( void ) {
	...
	const SERCOM_SPI_CTRLA_Type ctrla = { // Registry CTRLA
		.bit.DORD = 0, // MSB first   bit 30 Data order. indicates the data order when a character is shifted out from the Data register
		.bit.CPHA = 0, // Mode 0   bit 28  In combination with the Clock Polarity bit (CPOL), this bit determines the SPI transfer mode. The data is sampled on a leading SCK edge and changed on a trailing SCK edge.
		.bit.CPOL = 0,  // bit 29 Clock Polarity 0: SCK is low when idle. The leading edge of a clock cycle is a rising edge, while the trailing edge is a falling edge.
		.bit.FORM = 0, // SPI frame Format
		.bit.DIPO = 3, // MISO on PAD[3]  Data In Pinout SERCOM PAD[3] is used as data input 
		.bit.DOPO = 0, // MOSI on PAD[0], SCK on PAD[1], SS_ on PAD[2]  DOPO: Data Out Pinout
